  ...
  Annotator.Plugin.Categories = (function(_super) {
    ...
    Categories.prototype.AnnotationViewer = function(viewer, annotations) {
      ...
      if (annotations.length > 0) {
        for(i=0, len = annotations.length; i < len; i++) {
          ...
          $('ul.annotator-widget > li.annotator-item').prepend('<div style="vertical-align: middle;text-transform:capitalize;">'+annotation.category+'</div>'); 
        }
      }
    }
    ...
    Categories.prototype.AnnotationCategory = function(editor,annotation) {
      ...
      if ($('li.annotator-radio').length == 0) { //If the category section not exists
        ...
        for (cat in _categories) {
          ...
          radio = radio + '<label for="'+cat+'" style="vertical-align: middle;text-transform:capitalize;"><div class="'+_categories[cat]+' square" style="display:inline-block;height:15px;width:30px;margin-top:3px;margin-bottom:3px;margin-rigth:5px;vertical-align:middle"></div><span style="margin-left:5px">'+cat+'</span></label><br/>';
          i = i + 1;
    ...
    Categories.prototype.updateViewer = function(field, annotation) {
      field = $(field);
      field.html('<span class="annotator-hl-' + annotation.category + '">' +annotation.category.toUpperCase() + '</span>').addClass('annotator-hl-'+ annotation.category );
